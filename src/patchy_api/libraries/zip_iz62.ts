...
const config = {};
var HAS_TYPED = config.HAS_TYPED = typeof Uint8Array !== 'undefined' && typeof Uint16Array !== 'undefined';
...
config.CAN_CHARCODE_APPLY = CAN_CHARCODE_APPLY;
config.CAN_CHARCODE_APPLY_TYPED = CAN_CHARCODE_APPLY_TYPED;

// Function.prototype.apply stack max range
var APPLY_BUFFER_SIZE = config.APPLY_BUFFER_SIZE = 65533;
config.APPLY_BUFFER_SIZE_OK = null;
...
config.STRING_LASTINDEXOF_BUG = STRING_LASTINDEXOF_BUG;


var BASE62TABLE = config.BASE62TABLE = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';

// Buffers
var TABLE_LENGTH = config.TABLE_LENGTH = BASE62TABLE.length;
var TABLE_DIFF = Math.max(TABLE_LENGTH, 62) - Math.min(TABLE_LENGTH, 62);
config.BUFFER_MAX = TABLE_LENGTH - 1;
//var TABLE_BUFFER_MAX = BUFFER_MAX * (BUFFER_MAX + 1);

// Sliding Window
var WINDOW_MAX = config.WINDOW_MAX = 1024;
config.WINDOW_BUFFER_MAX = 304; // maximum 304

// Chunk buffer length
var COMPRESS_CHUNK_SIZE = config.COMPRESS_CHUNK_SIZE = APPLY_BUFFER_SIZE;
config.COMPRESS_CHUNK_MAX = COMPRESS_CHUNK_SIZE - TABLE_LENGTH;
var DECOMPRESS_CHUNK_SIZE = config.DECOMPRESS_CHUNK_SIZE = APPLY_BUFFER_SIZE;
config.DECOMPRESS_CHUNK_MAX = DECOMPRESS_CHUNK_SIZE + WINDOW_MAX * 2;

// Unicode table : U+0000 - U+0084
var LATIN_CHAR_MAX = 11;
config.LATIN_BUFFER_MAX = LATIN_CHAR_MAX * (LATIN_CHAR_MAX + 1);

// Unicode table : U+0000 - U+FFFF
var UNICODE_CHAR_MAX = config.UNICODE_CHAR_MAX = 40;
config.UNICODE_BUFFER_MAX = UNICODE_CHAR_MAX * (UNICODE_CHAR_MAX + 1);

// Index positions
config.LATIN_INDEX = TABLE_LENGTH + 1;
config.LATIN_INDEX_START = TABLE_DIFF + 20;
config.UNICODE_INDEX = TABLE_LENGTH + 5;

// Decode/Start positions
config.DECODE_MAX = TABLE_LENGTH - TABLE_DIFF - 19;
var LATIN_DECODE_MAX = config.LATIN_DECODE_MAX = UNICODE_CHAR_MAX + 7;
var CHAR_START = config.CHAR_START = LATIN_DECODE_MAX + 1;
var COMPRESS_START = config.COMPRESS_START = CHAR_START + 1;
var COMPRESS_FIXED_START = config.COMPRESS_FIXED_START = COMPRESS_START + 5;
config.COMPRESS_INDEX = COMPRESS_FIXED_START + 5; // 59
// Currently, 60 and 61 of the position is not used yet
const util = {};
util.createBuffer = function (bits, size) {
	if (!config.HAS_TYPED) {
...
var truncateBuffer = util.truncateBuffer = function (buffer, length) {
...
util.bufferToString_fast = function (buffer, length) {
	if (length == null) {
...
var bufferToString_chunked = util.bufferToString_chunked = function (buffer) {
	var string = '';
...
var bufferToString_slow = util.bufferToString_slow = function (buffer) {
	var string = '';
	var length = buffer.length;
