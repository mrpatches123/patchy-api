	...
	callback: (sender, args) => {
		...
		switch (subCommand) {
			case 'add': {
				...
				if (full) return sender.sendMessage(`${plotKey} is full`);
				if (!wasAdded && plotNumber !== plotNumberToSet) return sender.sendMessage(`${playerName} on ${plotKey} already has plot Number ${plotNumber}`);
				if (!wasAdded) return sender.sendMessage(`${playerName} on ${plotKey} could not be set to a plotNumber of ${plotNumber} as it is taken.`);
