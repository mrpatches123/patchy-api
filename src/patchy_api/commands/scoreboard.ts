...
commandBuilder.register('scoreboard', {
	...
	callback: (sender, args) => {
		...
		if (!objective && subcommand !== 'print') return sender.sendMessage(`objective at params[1] is not defined`);
		let player;
		if (playerName) {
			player = players.find({ name: playerName });
			if (!player) return sender.sendMessage(`player: ${playerName}, at params[2] does not exist!`);
			if (!Number.isInteger(number)) return sender.sendMessage(`value: ${playerName}, at params[3] id not defined!`);
		}
		switch (subcommand) {
			case 'print': {
