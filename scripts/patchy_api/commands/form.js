...
commandBuilder.register('forms', {
    ...
    callback: (sender, args) => {
        ...
        if (!key)
            return sender.sendMessage(JSON.stringify(formBuilder, (key, value) => (value instanceof Function) ? '<f>' : value, 4));
        if (!formBuilder.hasOwnProperty(key))
            return sender.sendMessage(`Key: ${key}, doesn't exsist!`);
        switch (subcommand) {
            case 'print':
