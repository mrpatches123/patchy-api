import { commandBuilder, global } from '../modules.js';
import config from '../config.js';
const { commandPrefix: prefix } = config;
global.unbannWindows ??= [];
commandBuilder.register('unban', {
    ...
    callback: (sender, args) => {
        ...
        switch (subcommand) {
            case 'add': {
                if (global.unbannWindows.includes(playerName))
                    return sender.sendMessage(`You already opened an unban window for $playerName}!`);
                global.unbanWindows.push(playerName);
                return sender.sendMessage(`You opened an unban window for ${playerName}!`);
            }
            case 'remove': {
                if (!global.unbanWindows.includes(playerName))
                    return sender.sendMessage(`You have not opened an unban window for $playerName}!`);
