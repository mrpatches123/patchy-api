	...
	callback: (sender, args) => {
		...
		switch (subcommand) {
			case 'add': {
				...
				if (!global.unbanWindows.includes(playerName)) return sender.tell(`You have not opened an unban window for $playerName}!`);
				global.unbanWindows = global.unbanWindows.filter(name => name !== playerName);
				return sender.tell(`You removed an unban window for ${playerName}!`);
			}
		}
	}
});