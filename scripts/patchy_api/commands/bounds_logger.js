...
commandBuilder.register('bound', {
    ...
    callback: (sender, args) => {
        ...
        if (!subCommand)
            return sender.sendMessage('forgot subCommand');
        if (!key)
            return sender.sendMessage('forgot key');
        switch (subCommand) {
            case 'add': {
                if (!bounds.hasOwnProperty(id))
                    bounds[id] = {};
                if (!bounds[id].hasOwnProperty(key))
                    bounds[id][key] = [{}, {}];
...
//# sourceMappingURL=bounds_logger.js.map