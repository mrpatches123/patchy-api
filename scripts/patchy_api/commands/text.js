import { commandBuilder, content, relativeParse } from "../modules.js";
...
commandBuilder.register('text', {
    ...
    callback: (sender, args) => {
        ...
        if (!subCommand)
            return sender.sendMessage(`subCommand: ${subCommand}, at args[0] is not defined `);
        if (!x)
            return sender.sendMessage(`text: ${x}, at args[1] is not defined `);
        if (!y)
            return sender.sendMessage(`text: ${y}, at args[2] is not defined `);
        if (!z)
            return sender.sendMessage(`text: ${z}, at args[3] is not defined `);
        if (!name)
            return sender.sendMessage(`text: ${name}, at args[4...] is not defined `);
        ...
        switch (subCommand) {
            ...
            case 'delete': {
                ...
                if (!entity)
                    return sender.sendMessage(`No text entity found at ${x} ${y} ${z}!`);
