...
import { content, obfuscate255, deobfuscate255, chunkStringBytes } from '../utilities.js';
...
class TagDatabases {
    ...
    initalizeAll() {
        ...
        this.initalized ??= {};
        Object.entries(global.players).forEach(([id, player]) => {
    ...
    getTestRaw(player, databaseId) {
        ...
        const objectDatabase = {};
        rawDatabases.forEach(([databaseId, order, value]) => {
            if (!objectDatabase.hasOwnProperty(databaseId))
                objectDatabase[databaseId] = [];
            objectDatabase[databaseId].push([order, value]);
        });
        if (databaseId)
            return objectDatabase[databaseId];
    ...
    queueSave(player, databaseId) {
        ...
        const { id } = player;
        if (!this.hasOwnProperty(databaseId))
            return new Error(`databaseId: ${databaseId}, does not exist on tag database`);
        if (!this.__queuedSaves.saves.hasOwnProperty(databaseId))
            this.__queuedSaves.saves[databaseId] = {};
        if (this.__queuedSaves.saves[databaseId].hasOwnProperty(id))
            return;
        if (!this.__queuedSaves.saves[databaseId].hasOwnProperty(id))
            this.__queuedSaves.saves[databaseId][id] = player;
        if (!this.__queuedSaves.subscribed) {
            eventBuilder.subscribe('end_tagSaveQueue*API', {
                tickAfterLoad: () => {
                    if (!Object.keys(this.__queuedSaves.saves).length)
                        return eventBuilder.unsubscribe('end_tagSaveQueue*API', 'tickAfterLoad');
    ...
    save(databaseId, player) {
        ...
        players.forEach(player => {
            const { id } = player;
            if (!this.databases?.[databaseId]?.[id])
                return;
            const tags = player.getTags();
            let rawTexts;
            if (tags.length)
                rawTexts = tags.filter(text => text.startsWith('tagDB:') && deobfuscate255(text.slice('tagDB:'.length)).match(/(.*):(\d+):/).splice(1)[0] === databaseId);
            if (!this.databases.hasOwnProperty(databaseId))
                this.databases[databaseId] = {};
            if (!this.databases[databaseId].hasOwnProperty(id))
                this.databases[databaseId][id] = new Database();
            if (rawTexts && rawTexts.length)
                rawTexts.forEach(tag => player.removeTag(tag));
...
//# sourceMappingURL=tag_database.js.map