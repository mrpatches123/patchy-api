        ...
        Object.entries(this.subscriptions.keys).forEach(([key, { suppressed, callback: callbackForKey }], i) => {
            try {
                ...
                if (callback instanceof Function)
                    callback(key, eventResponse, callbackForKey, i);
                else
                ...
                if (!eventBuilder.subscriptions.hasOwnProperty(this.eventKey))
                    return;
                if (!eventBuilder.subscriptions[this.eventKey].keys.hasOwnProperty(key))
                    return;
