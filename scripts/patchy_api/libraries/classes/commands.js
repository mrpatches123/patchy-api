...
import { parseCommand } from "../utilities.js";
...
class CommandBuilder {
    ...
    invalidSyntax(command, sender, prefix, error) {
        ...
        sender.sendMessage({ "rawtext": [{ "text": "§c" }, { "translate": "commands.generic.syntax", "with": [`${prefix}${command} `, `${error[0]}`, `${error.filter((item, i) => { if (i !== 0)
                            return item; }).join(" ")}`] }] });
    }
    getPrefixs() {
            ...
            if (requires) {
                ...
                if (score) {
                    Object.entries(score).forEach(([objective, value]) => {
                        ...
                        if (sender.scores[objective] !== value) {
                            notPermissions = true;
                        }
                    });
                }
                if (tag) {
                    ...
                    Object.entries(tag).forEach(([tag]) => { if (tags.includes(tag)) {
                        notPermissions = true;
                    } });
                }
                if (notPermissions) {
    ...
    register(command, { description = '', aliases = [], usages = {}, requires = false, prefix = config.commandPrefix, callback }) {
        ...
        if (typeof description !== 'string')
            throw new Error('Description must to be a string.');
        else if (requires !== false && typeof requires !== 'object' && !isArray(requires))
            throw new Error('Requires must to be false or a object.');
        else if (typeof usages !== 'object')
            throw new Error('usages must to be a object or an array');
        else if (typeof callback !== 'function')
            throw new Error('Callback must to be a Function.');
    ...
    getInfo(sender, prefix, command, subCommand, permissions = true) {
        const argsLength = arguments.length;
        ...
        if (this.commandsObject[prefix]) {
            ...
            if (!command)
                return this.invalidSyntax(`help`, sender, command, ['']);
        }
    }
    ;
    getCommandFromAliases(prefix, aliase) {
        if (!this.commandsObject[prefix])
            return;
