...
import { isVector3, orArray, sort3DVectors, content } from "../../utilities";
...
export class StructureBuilder {
    ...
    getSaveStatus(id) {
        if (!this.saves.hasOwnProperty(id))
            return;
        return this.saves[id];
    }
    getLoadStatus(id) {
        if (!this.loads.hasOwnProperty(id))
            return;
    ...
    save(options) {
        ...
        if (!(dimension instanceof Dimension))
            throw new Error('dimension in saveOptions at params[0] is not of type: Dimension!');
        if (!isVector3(location1))
            throw new Error('location1 in saveOptions at params[0] is not of type: Vector3!');
        if (!isVector3(location2))
            throw new Error('location2 in saveOptions at params[0] is not of type: Vector3!');
        if (saveMode && !saveModes.includes(saveMode))
            throw new Error(`saveMode in saveOptions at params[0] is defined and not one of the following: ${orArray(saveModes)}!`);
        if (includesEntites && typeof includesEntites !== 'boolean')
            throw new Error(`includesEntites in saveOptions at params[0] is defined and not of type: Boolean!`);
        if (includesBlocks && typeof includesBlocks !== 'boolean')
            throw new Error(`includesBlocks in saveOptions at params[0] is defined and not of type: Boolean!`);
        ...
        function tick() {
            const systemIdNumber = system.run(async () => {
                try {
                    ...
                    if (!currentSave)
                        return (system.clearRun(systemIdNumber), thisStructure.subscribedSaveQueue = false);
                    ...
                    if (shouldReturn)
                        return (shouldReturn = false, tick());
                    if (done)
                        return (tick(), thisStructure.saves[id].done = true, thisStructure.saveQueue.shift(), dimension.runCommandAsync(`tickingarea remove StructureSaveAPI`).catch(error => console.warn(error, error.stack)));
        ...
        function tick() {
            const systemIdNumber = system.run(async () => {
                try {
                    ...
                    if (!currentLoad)
                        return (system.clearRun(systemIdNumber), thisStructure.subscribedLoadQueue = false);
                    ...
                    if (tickRemoveResult?.successCount)
                        return (tick(), thisStructure.vars.tickCleared = true);
                    ...
                    content.warn({ tickAddResult: tickRemoveResult?.successCount });
                    if (tickRemoveResult?.successCount)
                        return tick();
                    ...
                    content.warn({ structureLoadResult: tickRemoveResult?.successCount });
                    if (tickRemoveResult?.successCount) {
                        tick();
                        thisStructure.vars.newRow = false;
    ...
    load(options) {
        ...
        if (typeof name !== 'string')
            throw new Error(`name in loadOptions at params[0] is defined and not of type: String!`);
        if (!(dimension instanceof Dimension))
            throw new Error('dimension in loadOptions at params[0] is not of type: Dimension!');
        if (!isVector3(location))
            throw new Error('location in loadOptions at params[0] is not of type: Vector3!');
        if (rotation && !rotations.includes(rotation))
            throw new Error(`rotation in loadOptions at params[0] is defined and not one of the following: ${orArray(rotations)}!`);
        if (mirror && !mirrors.includes(mirror))
            throw new Error(`mirror in loadOptions at params[0] is defined and not one of the following: ${orArray(mirrors)}!`);
        if (animationMode && !animationModes.includes(animationMode))
            throw new Error(`animationMode in loadOptions at params[0] is defined and not one of the following: ${orArray(animationModes)}!`);
        if (animationSeconds && typeof animationSeconds !== 'number')
            throw new Error(`animationSeconds in loadOptions at params[0] is defined and not of type: Number!`);
        if (includesEntites && typeof includesEntites !== 'boolean')
            throw new Error(`includesEntites in loadOptions at params[0] is defined and not of type: Boolean!`);
        if (includesBlocks && typeof includesBlocks !== 'boolean')
            throw new Error(`includesBlocks in loadOptions at params[0] is defined and not of type: Boolean!`);
        if (waterlogged && typeof waterlogged !== 'boolean')
            throw new Error(`waterlogged in loadOptions at params[0] is defined and not of type: Boolean!`);
        if (seed && typeof seed !== 'string')
            throw new Error(`seed in loadOptions at params[0] is defined and not of type: String!`);
