...
class RequestBuilder {
    ...
    addMemory(id, key, target, type, value) {
        if (typeof id !== 'string' && typeof id !== 'number')
            throw new Error(`id: ${id}, at params[0] is not a String or Number!`);
        if (typeof key !== 'string' && typeof key !== 'number')
            throw new Error(`key: ${key}, at params[1] is not a String or Number!`);
        if (typeof target !== 'string' && typeof target !== 'number')
            throw new Error(`target: ${target}, at params[2] is not a String or Number!`);
        if (typeof type !== 'string' && typeof type !== 'number')
            throw new Error(`type: ${type}, at params[3] is not a String or Number!`);
        if (value === undefined)
            throw new Error(`value at params[4] is not defined`);
        if (!this.memory.hasOwnProperty(id))
            this.memory[id] = {};
        if (!this.memory[id].hasOwnProperty(key))
            this.memory[id][key] = {};
        if (!this.memory[id][key].hasOwnProperty(target))
            this.memory[id][key][target] = {};
        if (!this.memory[id][key][target].hasOwnProperty(type))
            this.memory[id][key][target][type] = {};
    ...
    getMemoryTarget(id, target, keys, type, isArray = false) {
        ...
        if (typeof target !== 'string' && typeof target !== 'number')
            throw new Error(`target: ${target}, at params[0] is not a String or Number!`);
        if (keys) {
            if (!(keys instanceof Array))
                throw new Error(`keys, at params[1] is not an Array!`);
            keys.forEach((key, i) => {
                if (typeof key !== 'string' && typeof key !== 'number')
                    throw new Error(`keys: ${key}, at params[1] at index[${i}] is not a String or Number!`);
            });
        }
        if (!keys) {
            const output = {};
            Object.entries(this.memory[id] ?? {}).forEach(([key, targets]) => {
                ...
                if (!target)
                    return;
                if (type && target.hasOwnProperty(type))
                    output[key] = target[type];
                else
                    output[key] = target;
            });
            return output;
            ...
            const output = {};
            keys.forEach(key => {
                if (!this.memory?.[id]?.[key]?.[target])
                    return;
                if (type && this.memory[id][key][target].hasOwnProperty(type))
                    return output[key] = this.memory[id][key][target];
                else
                    output[key] = this.memory[id][key][target];
            });
        }
    }
    ...
    add(id, key, target, type, value) {
        ...
        if (!requests.hasOwnProperty(key))
            requests[key] = {};
        if (!requests[key].hasOwnProperty(target))
            requests[key][target] = {};
        if (!requests[key][target].hasOwnProperty(type))
            requests[key][target][type] = {};
     ...
     */
    watch(id, testCallback, findCallback, options) {
        content.warn({ t: 'RequestBuilderwatch', id });
        if (typeof id !== 'string')
            throw new Error(`findCallback for id: ${id}, is not a String`);
        if (!(testCallback instanceof Function))
            throw new Error(`testCallback for id: ${id}, is not a Function.`);
        if (findCallback && !(findCallback instanceof Function))
            throw new Error(`findCallback for id: ${id}, is defined and not a Function`);
        if (options && !(options instanceof Object))
            throw new Error(`findCallback for id: ${id}, is defined and not an Object`);
        ...
        if (removeKey !== null && removeKey !== undefined && !(typeof removeKey === 'boolean'))
            throw new Error(`removeKey for options for id: ${id}, is not undefined, null or a boolean`);
        ...
        const call = () => {
            ...
            Object.entries(requests).forEach(([key, targets]) => {
                Object.entries(targets).forEach(([target, types]) => {
                    Object.entries(types).forEach(([type, value]) => {
                        ...
                        if (test) {
                            if (findCallback)
                                findCallback(key, target, type, value);
                            if (removeKey) {
            ...
            eventBuilder.subscribe(`requestsAPI*${id}`, {
                ...eventKeys.reduce((sum, eventkey, i) => { sum[eventkey] = call; return sum; }, {})
            });
        };
    }
    /**
