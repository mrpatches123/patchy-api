            ...
            tick: {
                function: ({ currentTick }) => {
                    ...
                    players.forEach((player, i) => {
                        ...
                        if (players.length === i + 1 && !global.hasOwnProperty('loadedPlayers'))
                            return;
                        global.loadedPlayers = true;
        ...
        subscription: {
            beforeDataDrivenEntityTriggerEvent: {
                function: (event) => {
                    ...
                    eventBuilder.getEvent('beforeDataDrivenPlayerTriggerEvent').iterate({ cancel, player: entity, getModifiers() { return event.getModifiers(); }, setModifiers(modifiers) { event.setModifiers(modifiers); }, get modifiers() { return event.getModifiers(); }, set modifiers(modifiers) { event.setModifiers(modifiers); }, id });
                },
                options: { entityTypes: ["minecraft:player"] }
            }
        }
...
//# sourceMappingURL=events_first.js.map