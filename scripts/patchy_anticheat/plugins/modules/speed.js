      ...
      if (current !== value) {
        ...
        if (nukerFlags > toggles.speedFlagsKick) {
          ...
          if (kicks > toggles.kicksBanS) {
            ...
            // player.runCommandAsync(`say ${JSON.stringify(playerAC)}`);
            try { overworld.runCommandAsync(`tellraw @a[scores={Notifications=1}] {"rawtext":[{"text":"§l§f[§9PAC§f] §7${name} §4failed §1Speed §fby §4modifying§7 the minecraft:movement Component §fand was §1Banned §fdue to §7${kicks}>${toggles.kicksBanS} §1Kicks!"}]}`); } catch { }
            ...
            try { overworld.runCommandAsync(`tellraw @a[scores={Notifications=1}] {"rawtext":[{"text":"§l§f[§9PAC§f] §7${name} §4failed §1Speed §fby §4modifying§7 the minecraft:movement Component §fand was §1Kicked §7${kicks} §ftimes due to §7${speedFlags}>${toggles.speedFlags} §1Flags!"}]}`); } catch { }
          }
        } else {
          let s = (nukerFlags !== 1) ? 's' : '';

          try { overworld.runCommandAsync(`tellraw @a[scores={Notifications=1}] {"rawtext":[{"text":"§l§f[§9PAC§f] §7${name} §4failed §1Speed §fby §4modifying§7 the minecraft:movement Component §fand was §1Flagged §7${speedFlags} §ftime${s}!"}]}`); } catch { }
        }
